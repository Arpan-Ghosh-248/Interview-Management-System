  ...
  class Create {
    ...
    public function AddCandidate($data){
      if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['addCand1'])) {
        ...
        if (empty($candName) or empty($candEmail) or empty($candQuali) ){
        ...
        $sql = "insert into newcandidates1(cand_name,mobileno,candEmail,Media,Appliedfor,candQuali,CallingPerson,modeofinterview,interviewscheduleddate,time,Result,InterviewScheduledBy,Comments) values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $arr = array($candName,$mobileno, $candEmail, $Media, $Appliedfor, $candQuali, $CallingPerson,$modeofinterview,$interviewscheduleddate,$time,$Result,$InterviewScheduledBy,$Comments);
